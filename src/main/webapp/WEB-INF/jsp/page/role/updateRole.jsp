	...
	<div class="box-body">
		...
		<form:form method="POST" commandName="role" id="roleF"
			...
			<div class="form-group">
				<label for="nameField" class="col-sm-2 control-label esoft-left">nom de role:&nbsp;<span class="error">*</span></label>
				<div class="col-sm-4">
			...
			<div class="form-group">
				<label for="DescriptionField" class="col-sm-2 control-label esoft-left">Permissions:</label>
				<div class="col-sm-4"> 
				
				<form:select path="permissions" multiple="multiple" cssClass="tokenize-sample  " id="permissions">
				<form:options  items="${role.permissions}" itemLabel="label" itemValue="id"  selected="selected"/>
				</form:select>
...
<%-- 						items="${permissions}" itemLabel="label" itemValue="id" /> --%>
			...
			<form:hidden path="id"/>
			<div class="form-group">
				<button type="reset" class="btn-sm btn btn-danger btn pull-right " onclick="location.href='${baseURL}/role/list'" style="margin-right: 2%;">Annuler</button>
				<button type="submit" class="btn-sm btn btn-primary btn pull-right"
					style="margin-right: 2%;"><i class="fa fa-plus"></i>&nbsp;Ajouter</button>
			</div>
		</form:form>
	</div>
</div>
