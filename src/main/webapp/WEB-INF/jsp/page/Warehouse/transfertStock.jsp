...
<div class="box box-solid box-primary">
	<div class="box-header">
		<h3 class="box-title">transfert de stock </h3>
	</div>
	<!-- /.box-header -->
	<!-- form start -->
	...
	<div class="box-body">
		<form:form method="POST" commandName="transfert" id="correctF"
			...
			<div class="form-group">
				<label for="warehouseSource"
					class="col-sm-2 control-label esoft-left">Entrepôt
					source:&nbsp;<span class="error">*</span>
				</label>
				<div class="col-sm-2" id="select-war1">
					<form:select path="source" cssClass="tokenize-sample mono-select "
						id="warehouse-source" size="1">
						<c:if test="${transfert.source.id >0}">
							<form:option value="${transfert.source.id}" selected="selected">
								<c:out value="${transfert.source.name}" />
							</form:option>
						</c:if>
					</form:select>
				...
				$("#select-war1").on("focus", ".tokenize-sample ", function() {
				    console.log($("#select-war1 .Token span").text());
				   $text= $("#select-war1 .Token span").text();
				   $(" #select-war1 .Token").remove();
				   $("#select-war1 select option[selected='selected']").remove();
				    $("#select-war1 .TokenSearch input").val($text);
				});
				</script>
