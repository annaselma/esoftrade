...
<div class="box box-solid box-primary">
	<div class="box-header">
		<h3 class="box-title">modifier commande Client</h3>
	</div>
	<!-- /.box-header -->
	<!-- form start -->
...
<span class="error">Veuillez saisir les champs obligatoire (*)</span>
		<form:form method="POST" commandName="order" id="orderF"
			...
			<div class="form-group">
				<label for="productField" class="col-sm-2 control-label esoft-left">Client:&nbsp;<span
					class="error">*</span></label>
				<div class="col-sm-4" id="select-company">
					<form:select path="company" cssClass="tokenize-sample mono-select "
						id="company" size="1">
						<c:if test="${order.company.id >0}">
							<form:option value="${order.company.id}" selected="selected">
			...
			<script type="text/javascript">
					$('#company').tokenize({
						"newElements" : false,
						maxElements : 1,
						datas : "${baseURL}/third/search?type=customer",
						valueField : "id",
						textField : "name"
					});
					$("#select-company")
			...
			<div class="form-group">
				...
				<div class="form-group">
				<label for="statusField" class="col-sm-2 control-label esoft-left">méthode d'expidition:&nbsp;</label>
				<div class="col-sm-4" id="select-delivry">
					<form:select path="shippingMethod" cssClass="form-control ">
						<option value="transporter">Transporteur</option>
						<option value=" customerPickUp">Enlevement par Client</option>
					</form:select>
					<form:errors path="shippingMethod" cssClass="error" />
				</div>
				</div>
				<hr>
